...
const Home = () => {
  ...
  const [consultaParaCancelar, setConsultaParaCancelar] = useState<Consulta | null>(null);
  ...
  const loadTodasConsultas = async () => {
    try {
      // Troca para o endpoint correto!
      const consultasData = await ConsultaApiService.getProximasConsultas('voluntario');
      const consultasConvertidas: Consulta[] = consultasData.map(consulta => ({
        id: consulta.idConsulta,
        profissional: consulta.voluntario?.ficha?.nome || "Profissional não informado",
        especialidade: consulta.especialidade?.nome || "Especialidade não informada",
      ...
      <div className="min-h-screen w-full flex flex-col md:flex-row bg-[#EDF2FB] dark:bg-gradient-to-br dark:from-[#181A20] dark:via-[#23272F] dark:to-[#181A20] transition-colors duration-300 font-sans text-base">
        ...
        <div className={`transition-all duration-500 ease-in-out
          ...
          <SidebarMenu className="gap-4 text-sm md:text-base">
            ...
            {/* Botão de Sair permanece o mesmo */}            <SidebarMenuItem>
              <Tooltip>
                <TooltipTrigger asChild>
                              ...
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <div className="space-y-2">
                                  <div className="flex justify-between items-center">
                                    <span className="font-medium text-gray-800 dark:text-gray-200">{proximaConsulta?.assistido?.ficha?.nome} {proximaConsulta?.assistido?.ficha?.sobrenome}</span>
                                    ...
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <Button
                                          ...
                                          onClick={() => {
                                            const consulta = proximasConsultas.find(
                                              c => c.data.toDateString() === consultasSummary.proxima?.toDateString()
                                            );
                                            if (consulta) abrirModalCancelamento(consulta);
                                          }}
                                        >
                                  ...
                                  <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="font-medium text-gray-800 dark:text-gray-200">
                                        {consulta.assistido?.ficha?.nome} {consulta.assistido?.ficha?.sobrenome}
                                      </span>
                                    </div>
          ...
          <DialogContent className="w-[95vw] max-w-[600px] max-h-[90vh] overflow-y-auto bg-white dark:bg-[#23272F] border border-[#EDF2FB] dark:border-[#444857] mx-4 sm:mx-auto">
            <DialogHeader className="pb-4">
              <DialogTitle className="text-lg sm:text-xl font-semibold text-indigo-900 dark:text-gray-100 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ED4231" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
                Detalhes da Consulta
              </DialogTitle>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800">
                    <h4 className="font-semibold text-indigo-900 dark:text-indigo-100 mb-3 flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="7" r="4" /><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /></svg>
                      Paciente
                    </h4>
                    <div className="space-y-2 text-sm">
                      <p><strong>Nome:</strong> {consultaDetalhes.assistido?.ficha?.nome} {consultaDetalhes.assistido?.ficha?.sobrenome}</p>
                      <p><strong>Email:</strong> {consultaDetalhes.assistido?.email || 'Não informado'}</p>
                    </div>
                  </div>
                  {/* Card da Consulta */}
