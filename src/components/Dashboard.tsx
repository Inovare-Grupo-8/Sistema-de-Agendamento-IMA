...
const Dashboard = () => {
  ...
  const [voluntarioProfile, setVoluntarioProfile] = useState<VoluntarioProfile>(
    defaultVoluntarioProfile
  );
    ...
    const loadUserData = async () => {
      try {
        ...
        const safeProfile = (userProfile ??
          {}) as Partial<VoluntarioProfile> & { fotoUrl?: string };
  ...
  const displayName =
    `${voluntarioProfile.nome} ${voluntarioProfile.sobrenome}`.trim() ||
    "Voluntário";
      ...
      <div className="min-h-screen w-full flex flex-col md:flex-row bg-background">
        {!sidebarOpen && (
          <div className="w-full flex justify-start items-center gap-3 p-4 fixed top-0 left-0 z-30 bg-white/80 dark:bg-gray-900/90 shadow-md backdrop-blur-md">
            <Button
              ...
              className="p-2 rounded-full bg-primary text-white focus:outline-none shadow-md"
        ...
        >
          ...
          <div className="flex flex-col items-center gap-2 mb-8">
            ...
            />
            <span className="font-extrabold text-xl text-foreground tracking-wide">
              {displayName}
            </span>
