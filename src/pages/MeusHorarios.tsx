...
import { Calendar as CalendarIcon, Edit, Trash2, X, Menu, LogOut } from "lucide-react";
import { useThemeToggleWithNotification } from "@/hooks/useThemeToggleWithNotification";
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from "@/components/ui/select";

const ALL_TIMES = ["08:00","09:00","10:00","11:00","13:00","14:00","15:00","16:00","18:00","19:00","20:00","21:00"];

type DayAvailability = { times: string[]; modalidade: "ONLINE" | "PRESENCIAL"; local?: string };
...
const MeusHorarios = () => {
  ...
  const [dadosPessoais, setDadosPessoais] = useState<DadosPessoaisVoluntario>({ nome: "", sobrenome: "", email: "", telefone: "", dataNascimento: "" });
  ...
  const userId = useMemo(() => {
    ...
    if (stored) { try { const parsed = JSON.parse(stored); return parsed.idUsuario ?? "default"; } catch {} }
    return "default";
  }, []);
  const availabilityKey = (volId: string | number) => `availabilityVoluntario:${volId}`;
  ...
  useEffect(() => {
    buscarDadosPessoais().then((p) => { if (p) setDadosPessoais(p); }).catch(() => {});
    const raw = localStorage.getItem(availabilityKey(userId));
    if (raw) {
      try { setMap(JSON.parse(raw)); } catch { setMap({}); }
    }
    (async () => {
      const uid = typeof userId === 'number' ? userId : Number(userId);
      const list = await VoluntarioApiService.listarDisponibilidadesPorVoluntario(uid);
      if (list.length) {
        ...
        list.forEach(({ id, dataHorario }) => {
          ...
          if (dateStr && timeStr) {
            const entry = nextMap[dateStr] ?? { times: [], modalidade: 'ONLINE' };
            if (!entry.times.includes(timeStr)) entry.times.push(timeStr);
            entry.times = Array.from(new Set(entry.times)).sort((a,b)=>a.localeCompare(b));
        ...
        saveMap(nextMap);
        setIdsMap(nextIds);
        localStorage.setItem(`availabilityIds:${userId}`, JSON.stringify(nextIds));
      }
    })();
  ...
  const monthOptions = useMemo(() => {
    ...
    return Array.from(set)
      ...
      .map((m) => ({
        value: m,
        label: format(new Date(`${m}-01T00:00:00`), "MMMM yyyy", { locale: ptBR }),
      }));
  }, [sortedDates]);
  ...
  const hasResults = useMemo(() => {
    ...
    if (filterDay) {
      ...
      if (!day || !(filterTime ? day.times.includes(filterTime) : day.times.length > 0)) return false;
      return true;
    }
  ...
  useEffect(() => {
    const fetchDay = async () => {
      ...
      try {
        const uid = typeof userId === "number" ? userId : Number(userId);
        const horariosISO = await VoluntarioApiService.listarHorariosDisponiveisPorDia(filterDay, uid);
        const times = horariosISO.map((iso) => {
          const parts = String(iso).split("T");
          const clock = parts[1]?.substring(0,5) ?? "";
          return clock;
        }).filter(Boolean);
        const existing = map[filterDay] ?? { times: [], modalidade: "ONLINE" as const };
        const merged = Array.from(new Set([...(existing.times||[]), ...times])).sort((a,b)=>a.localeCompare(b));
  ...
  const removeTime = async (dateISO: string, t: string) => {
    ...
    if (!day) return;
    const next: DayAvailability = { ...day, times: day.times.filter((x) => x !== t) };
  ...
  const toggleEditTime = (t: string) => {
    setEditSelection((prev) => prev.includes(t) ? prev.filter((x) => x !== t) : [...prev, t].sort((a,b)=>a.localeCompare(b)));
  };

  const confirmEdit = async () => {
    if (!editDateISO) return;
    const uid = typeof userId === 'number' ? userId : Number(userId);
    const desired = Array.from(new Set(editSelection)).sort((a,b)=>a.localeCompare(b));
    const original = Array.from(new Set(editOriginal)).sort((a,b)=>a.localeCompare(b));
    ...
    for (let i = 0; i < moves; i++) {
      ...
      if (id) {
        const out = await VoluntarioApiService.atualizarDisponibilidade(id, `${editDateISO}T${newT}:00`, uid);
        if (out?.id) {
          const copy = { ...idsMap };
    ...
    for (const t of toAdd) {
      const out = await VoluntarioApiService.criarDisponibilidade(`${editDateISO}T${t}:00`, uid);
      if (out?.id) {
      ...
      <div className="min-h-screen w-full flex flex-col md:flex-row bg-background">
        <div className="bg-gradient-to-b from-white via-[#f8fafc] to-[#EDF2FB] dark:from-[#23272F] dark:via-[#23272F] dark:to-[#181A20] shadow-2xl rounded-2xl p-6 flex flex-col gap-6 overflow-hidden fixed md:static z-40 top-0 left-0 h-full md:h-auto border-r border-[#EDF2FB] dark:border-[#23272F] backdrop-blur-[2px] text-sm md:text-base w-4/5 max-w-xs md:w-72">
          <div className="flex flex-col items-center gap-2 mb-8">
            <ProfileAvatar profileImage={profileImage} name={displayName} size="w-16 h-16" className="shadow" />
            <span className="font-extrabold text-xl text-indigo-900 dark:text-gray-100 tracking-wide">{displayName}</span>
            <Badge className="bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300">Profissional</Badge>
          </div>
        ...
        <main className="flex-1 w-full md:w-auto mt-20 md:mt-0 px-2 md:px-6">
          <header className="w-full flex items-center justify-between px-4 md:px-6 py-4 bg-white/90 dark:bg-gray-900/95 shadow-md fixed top-0 left-0 z-20 backdrop-blur-md">
            <div className="flex items-center gap-3">
              ...
              <span className="font-extrabold text-foreground">Meus Horários</span>
            </div>
            <div className="flex items-center gap-3">
              <Button variant="outline" onClick={logout} className="px-4 py-2">Sair</Button>
            </div>
          </header>
          <div className="h-20" />

          <div className="max-w-5xl mx-auto w-full pb-8">
            <h1 className="text-2xl md:text-3xl font-bold text-indigo-900 dark:text-gray-100">Meus Horários Disponibilizados</h1>
            ...
            <Card className="rounded-2xl mt-4">
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input type="date" value={filterDay} onChange={(e)=>setFilterDay(e.target.value)} className="rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#23272F] px-3 py-2 text-sm" />
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Horário</span>
                  <Select onValueChange={(v)=>setFilterTime(v === 'ALL' ? '' : v)} value={filterTime || 'ALL'}>
                    <SelectTrigger>
                ...
                <div className="flex items-end gap-2">
                  ...
                  {loadingDay && <Badge className="bg-yellow-100 text-yellow-800">Carregando dia...</Badge>}
                </div>
              </CardContent>
            </Card>
            ...
            {filteredDates.length === 0 ? (
              <Card className="rounded-2xl mt-4">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-gray-500 mt-2">Nenhuma disponibilidade cadastrada</p>
                </CardContent>
              </Card>
            ) : (
              <>
                {hasFilters && !hasResults && (
                  <div className="mt-4 p-4 rounded-xl border-l-4 border-[#ED4231] bg-[#FFF7F5] text-gray-800 dark:bg-[#23272F] dark:text-gray-200" role="alert" aria-live="assertive">
                    <span className="font-medium">
                      {filterTime
                        ? `Nenhum horário disponibilizado em ${formatTimeRange(filterTime)} para o filtro selecionado.`
              ...
              <div className="mt-6 space-y-6">
                {filteredDates.map((iso) => {
                  ...
                  const day = map[iso];
                  const timesToShow = filterTime ? (day?.times || []).filter((t)=>t===filterTime) : (day?.times || []);
                  return (
                    <Card key={iso} className="rounded-2xl">
                      <CardHeader className="flex flex-row items-center justify-between py-4 bg-[#f8fafc] rounded-t-2xl">
                        <div className="flex items-center gap-2">
                          <CalendarIcon className="w-5 h-5 text-[#ED4231]" />
                          <CardTitle className="text-lg md:text-xl">{format(d, 'dd/MM/yyyy', { locale: ptBR })}</CardTitle>
                          <Badge className="ml-2 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">{day.modalidade === 'PRESENCIAL' ? 'Presencial' : 'Online'}</Badge>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button variant="outline" onClick={() => openEdit(iso)} className="h-9 px-3"><Edit className="w-4 h-4 mr-1" />Editar horários</Button>
                          <Button variant="outline" onClick={() => removeDay(iso)} className="h-9 px-3 text-red-600"><Trash2 className="w-4 h-4 mr-1" />Apagar dia</Button>
                        </div>
                      </CardHeader>
                      <CardContent className="p-4">
                        {timesToShow.length === 0 ? (
                          <span className="text-sm text-gray-500">Sem horários neste dia para este filtro</span>
                        ) : (
                          <div className="flex flex-wrap gap-2">
                            {timesToShow.sort((a,b)=>a.localeCompare(b)).map((t) => (
                              <div key={t} className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[#EDF2FB] dark:border-[#23272F] bg-white dark:bg-[#23272F] text-sm font-medium text-gray-800 dark:text-gray-200 shadow-sm">
                                <span>{formatTimeRange(t)}</span>
                                <Button variant="outline" size="icon" className="h-7 w-7" aria-label={`Remover ${t}`} onClick={() => removeTime(iso, t)}>
                                  <X className="w-4 h-4" />
                                </Button>
                              </div>
                            ))}
          ...
          <Dialog open={!!editDateISO} onOpenChange={(o)=>!o && setEditDateISO(null)}>
            <DialogContent className="max-w-lg">
              <DialogHeader><DialogTitle>Editar horários</DialogTitle></DialogHeader>
              <div className="grid grid-cols-3 gap-2">
                {ALL_TIMES.map((t) => {
                  const active = editSelection.includes(t);
                  return (
                    <Button key={t} variant={active ? 'default' : 'outline'} className={`h-10 px-3 text-sm rounded-xl ${active ? 'bg-[#ED4231] text-white border-2 border-[#ED4231]' : 'border-2 border-gray-300 bg-white text-gray-800 hover:border-[#ED4231] hover:bg-[#ED4231]/10'}`} onClick={()=>toggleEditTime(t)} aria-pressed={active}>
                      {formatTimeRange(t)}
                    </Button>
                  );
                })}
              ...
              <div className="flex justify-end gap-2 mt-4">
                <Button variant="outline" onClick={()=>setEditDateISO(null)}>Cancelar</Button>
                <Button onClick={confirmEdit} className="bg-[#ED4231] text-white">Salvar alterações</Button>
              </div>
            </DialogContent>
          </Dialog>
        </main>
